...
- 网页可启动、停止、刷新二维码、快速登录、真正退出 QQ、切换自动启动和移除实例。
...
4. 在账号卡片中用手机 QQ 扫描二维码并确认。
5. 页面显示“已登录”后即可关闭网页；QQ 进程会继续运行。
...
- “启动”：只启动当前卡片对应的 LinuxQQ 主进程。
- “停止”：向该实例的精确主进程发送 `SIGTERM`，不退出 QQ 账号、不清空数据。
- “刷新二维码”：直接让该实例的 QQ 登录内核生成新二维码。
- “历史账号快速登录”：调用 QQ 内核的快速登录接口，并且只允许卡片对应的 QQ 号。
- “退出 QQ”：调用内核 `offline()`，让账号真正下线并回到登录状态。
- “移除实例”：要求实例先停止，只删除面板配置，不删除账号目录。
