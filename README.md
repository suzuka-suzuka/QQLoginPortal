...
QQ 启动后会先读取内核保存的历史登录信息。如果当前实例的 QQ 号可快速登录，面板会自动尝试恢复一次，成功后直接显示在线；没有可用凭据、恢复失败或等待登录结果超过 15 秒时，再生成二维码。恢复期间不会同时刷新二维码，也不会尝试历史列表中的其他 QQ 号。

内核支持时，面板会启用 `setRemerberPwd(true)`，让 QQ 自己保存后续登录所需的凭据。账号目录会跨重启保留，但旧版本没有留下有效凭据时仍需先扫码一次。QQ 要求设备验证或票据失效时也需要重新扫码，不能保证永久免扫码。“自动启动”控制面板启动时是否启动该 QQ；每次从网页手动启动 QQ 时同样会尝试恢复登录。
